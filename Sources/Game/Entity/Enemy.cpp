...
namespace Prizm
{
	...
	Enemy::Enemy(void) : _impl(std::make_unique<Impl>()) {}
	...
	void Enemy::LoadShader(const std::shared_ptr<Shader>& shader)
	{
		_impl->_shader = shader;
	}

	void Enemy::LoadTexture(const std::shared_ptr<Texture>& texture)
	{
