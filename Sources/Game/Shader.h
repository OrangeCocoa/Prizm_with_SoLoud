...
namespace Prizm
{
	...
	public:
		...
		Buffer CreateConstantBuffer(Microsoft::WRL::ComPtr<ID3D11Device>& device, ConstantBufferType& data)
		{
			BufferDesc buffer = {};
			...
			cb.Initialize(device.Get(), &data);

			return cb;
		}

		template<class ConstantBufferType>
		void UpdateConstantBuffer(Microsoft::WRL::ComPtr<ID3D11DeviceContext>& dc, Buffer& buffer,
			ConstantBufferType& cb, ShaderType type, 
			unsigned int register_slot, unsigned int buffer_num)
		{
			Microsoft::WRL::ComPtr<ID3D11Buffer> constant_buffer = buffer.buffer_data;
